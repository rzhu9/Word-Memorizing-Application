...
public class advancedSearch extends JFrame {
	 ...
	 */
	public advancedSearch() throws FileNotFoundException {
		setTitle("AdvancedSearch");
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				ArrayList<word> output = db.searchWord2(input.toLowerCase());
				String outputstr = "";
				for (int i=0; i< output.size(); i++){
					outputstr = outputstr + (output.get(i).getEnglish() + " " + output.get(i).getChinese()) + "\n";
				}
				if (output.size() ==0){
					textPane.setText("the word does not exist");
				}
				else {
					textPane.setText(outputstr);
				}
			}
		});
	}
